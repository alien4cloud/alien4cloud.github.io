...
# Comming soon: Topology portability

The topology portability level is defined by combining the portability level of all its components.
In the topology edition view, every component in the catalog will have a portability information. Then, the architect will be able to see progressively the impact of what he chose to add on the designed topology.  
Afterward, these informations will be use to match the topology against the available locations, so that the deployer would see the locations on which he can deploy.
