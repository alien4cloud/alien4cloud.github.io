...
export BACKUP_NAME=$0

# Directory in which alien stores it's disk base data.
export ALIEN_DIR=~/.alien
# Url to access ElasticSearch rest api.
export ES_URL="http://localhost:9200"
export ALIEN_URL="http://localhost:8080"
...
curl -XPOST "$ES_URL/_snapshot/alien_backup/$BACKUP_NAME/_restore"
...
# once restore is complete trigger the plugin reload
curl -XPOST "$ALIEN_URL//rest/plugin/reload"
{% endhighlight %}
